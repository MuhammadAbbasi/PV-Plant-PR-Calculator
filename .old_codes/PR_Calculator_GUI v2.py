...
class PRCalculatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Mazara 01 - PV Performance Ratio (PR) Calculator")
        ...
        # Define Color Palette (Sleek Dark Theme)
        self.bg_color = "#121212"
        self.card_bg = "#1e1e1e"
        self.accent_color = "#00adb5" # Cyan
        self.accent_hover = "#008f95"
        ...
        # Entry Field styling
        self.style.configure("TEntry", fieldbackground="#2c2c2c", background="#2c2c2c", foreground=self.text_color, borderwidth=0)
        self.style.map("TEntry", fieldbackground=[("active", "#363636"), ("focus", "#3d3d3d")])
        
        # Button styling
        self.style.configure("TButton", background=self.accent_color, foreground=self.text_color, borderwidth=0, font=("Segoe UI Semibold", 10), padding=8)
        self.style.map("TButton", background=[("active", self.accent_hover), ("disabled", "#333333")], foreground=[("disabled", "#666666")])
        
        self.style.configure("Action.TButton", background=self.success_color, foreground=self.text_color, font=("Segoe UI Semibold", 11, "bold"), padding=10)
        self.style.map("Action.TButton", background=[("active", "#388e3c")])
        ...
        self.style.configure("Treeview", background="#1e1e1e", foreground=self.text_color, fieldbackground="#1e1e1e", rowheight=24, borderwidth=0, font=("Segoe UI", 9))
        self.style.configure("Treeview.Heading", background="#2c2c2c", foreground=self.accent_color, font=("Segoe UI Semibold", 9, "bold"), borderwidth=0)
        self.style.map("Treeview", background=[("selected", self.accent_color)], foreground=[("selected", "#000000")])
        ...
        print(">>> PV Performance Ratio Calculator Engine Initialized.")
        print(">>> Ready to analyze single-day or batch monthly folders.")
    ...
    def create_layout(self):
        ...
        # Title Banner
        title_lbl = ttk.Label(main_frame, text="MAZARA 01 - PERFORMANCE RATIO CALCULATOR", style="Title.TLabel")
        ...
        lbl_sec_in = ttk.Label(inputs_card, text="Calculation Settings", style="Section.TLabel")
        lbl_sec_in.pack(anchor="w", pady=(0, 12))
        
        # Folder row
        folder_frame = ttk.Frame(inputs_card, style="Card.TFrame")
        folder_frame.pack(fill="x", pady=5)
        lbl_f = ttk.Label(folder_frame, text="Input Folder Path:", style="Card.TLabel")
        lbl_f.pack(anchor="w")
        ...
        btn_browse = ttk.Button(folder_entry_frame, text="Browse...", command=self.browse_folder)
        ...
        ttk.Label(date_frame, text="Date (YYYY-MM-DD):", style="Card.TLabel").pack(anchor="w")
        ...
        ttk.Label(pr_frame, text="PVSyst Monthly PR:", style="Card.TLabel").pack(anchor="w")
        ...
        ttk.Label(thresh_frame, text="Min Irradiance (W/m²):", style="Card.TLabel").pack(anchor="w")
        ...
        lbl_chk = tk.Label(
            chk_frame, 
            text="Force reprocess already calculated days (Batch mode)", 
            bg=self.card_bg, 
            fg=self.text_color, 
            font=("Segoe UI", 10)
        )
        ...
        # Action button
        self.btn_calculate = ttk.Button(inputs_card, text="Calculate Performance Ratio", style="Action.TButton", command=self.start_calculation)
        self.btn_calculate.pack(fill="x", pady=(5, 5))
        
        # Progress/Status
        self.lbl_status = ttk.Label(inputs_card, text="Ready. Select folder and click Calculate.", style="Card.TLabel", foreground=self.muted_text)
        ...
        lbl_sec_me = ttk.Label(self.metrics_card, text="Key Results Summary", style="Section.TLabel")
        ...
        ttk.Label(card_avg, text="AVG INVERTER PR", style="MetricLbl.TLabel").pack(anchor="center")
        self.lbl_avg_pr_val = ttk.Label(card_avg, text="-- %", style="MetricVal.TLabel")
        self.lbl_avg_pr_val.pack(anchor="center", pady=10)
        ttk.Label(card_avg, text="Average of 36 Comp PRs", style="Card.TLabel", font=("Segoe UI", 8), foreground=self.muted_text).pack(anchor="center")
        ...
        ttk.Label(card_comp, text="COMPENSATED RAW PR", style="MetricLbl.TLabel").pack(anchor="center")
        ...
        ttk.Label(card_comp, text="Plant total with losses", style="Card.TLabel", font=("Segoe UI", 8), foreground=self.muted_text).pack(anchor="center")
        ...
        ttk.Label(card_uncomp, text="UNCOMPENSATED PR", style="MetricLbl.TLabel").pack(anchor="center")
        ...
        ttk.Label(card_uncomp, text="Plant total actual energy", style="Card.TLabel", font=("Segoe UI", 8), foreground=self.muted_text).pack(anchor="center")
        
        # Irradiance summary line
        self.lbl_irrad_summary = ttk.Label(self.metrics_card, text="Total daily irradiation: -- kWh/m² (POA > 50 W/m²)", style="Card.TLabel", font=("Segoe UI Semibold", 9))
        self.lbl_irrad_summary.pack(anchor="w", pady=(15, 0))
        
        # Bottom Grid: Detailed Inverters (Card)
        bottom_card = ttk.Frame(main_frame, padding=15, style="Card.TFrame")
        bottom_card.pack(fill="both", expand=True, pady=(15, 0))
        
        lbl_sec_det = ttk.Label(bottom_card, text="Detailed 36 Inverters Compensated Performance Ratio", style="Section.TLabel")
        lbl_sec_det.pack(anchor="w", pady=(0, 10))
        
        # Table of Inverter PRs
        table_frame = ttk.Frame(bottom_card, style="Card.TFrame")
        ...
        cols = ("Inverter ID", "Transformer", "Nominal DC (kW)", "Actual Energy (kWh)", "Energy Loss (kWh)", "Compensated PR (%)")
        ...
        for c in cols:
            ...
            align = "center" if c != "Inverter ID" else "w"
            width = 110 if c != "Inverter ID" else 150
        ...
        self.btn_export = ttk.Button(export_frame, text="Export Complete Data to Excel...", state="disabled", command=self.export_to_excel)
        ...
        lbl_sec_log = ttk.Label(log_card, text="Execution Live Log Console", style="Section.TLabel")
    ...
    def browse_folder(self):
        selected_folder = filedialog.askdirectory(title="Select Month Folder (format YYYY MM)")
        if selected_folder:
            self.folder_path_var.set(selected_folder)
            self.lbl_status.config(text="Selected folder: " + os.path.basename(selected_folder), foreground=self.accent_color)
    ...
    def start_calculation(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "PVSyst PR and Irradiance threshold must be valid numbers!")
            return
        ...
        self.lbl_status.config(text="Calculating PR... please wait...", foreground=self.warn_color)
    ...
    def calculate_single_day(self, folder, date_str, pvsyst_pr, threshold, calcolo_folder=None):
        ...
        if missing_files:
            err_msg = f"The following required files are missing in folder '{os.path.basename(folder)}':\n"
        ...
        try:
            ...
            if os.path.exists(prev_day_folder):
                ...
                if prev_satac_file:
                    ...
                    print(f"[{date_str}] Loaded previous day SATAC meter readings from '{os.path.basename(prev_satac_file)}'.")
        except Exception as ex:
            print(f"[{date_str}] Warning: Could not load previous day SATAC meter readings: {ex}")
        ...
        for inv_id in self.dc_powers:
            ...
            if tx_name in ["TX1", "TX3"]:
                df_result[f"{inv_id}_dt_loss"] = np.where(
            ...
            df_result[f"{inv_id}_curt_loss"] = np.where(
            ...
            df_result[f"{inv_id}_loss"] = df_result[f"{inv_id}_dt_loss"] + df_result[f"{inv_id}_curt_loss"]
            
        for tx in ["TX1", "TX2", "TX3"]:
            loss_cols = [f"{tx}-INV-{i}_loss" for i in range(1, 13)]
            df_result[f"{tx}_Total_Loss"] = df_result[loss_cols].sum(axis=1)
        ...
        original_format_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "original_format")
        ...
        if not os.path.exists(daily_file_path):
            if template_file:
                print(f"[{date_str}] Copying clean daily Excel template: '{os.path.basename(template_file)}' -> '{daily_filename}'")
                shutil.copy(template_file, daily_file_path)
            else:
                raise FileNotFoundError("Pristine daily Excel template not found in original_format folder!")
        else:
            print(f"[{date_str}] Target daily file '{daily_filename}' already exists; updating in place.")
        ...
        try:
            ...
            print(f"[{date_str}] DEBUG: Opening daily workbook: {abs_daily_path}")
            wb_daily = excel_daily.Workbooks.Open(abs_daily_path, UpdateLinks=0)
            print(f"[{date_str}] DEBUG: Daily workbook opened successfully!")
            ...
            print(f"[{date_str}] DEBUG: Finished reading Column B (96 rows).")
            ...
            print(f"[{date_str}] DEBUG: Finished building calc_rows_ordered.")
            ...
            print(f"[{date_str}] DEBUG: Writing dates in Column A of ws_calc...")
            ws_calc.Range("A15:A110").Value = date_col_vals
            print(f"[{date_str}] DEBUG: Writing dates in Column A of ws_inv...")
            ws_inv.Range("A15:A110").Value = date_col_vals
            print(f"[{date_str}] DEBUG: Finished writing dates in Column A. Writing PR_Calc data columns...")
            ...
            print(f"[{date_str}] DEBUG: Wrote WS_Calc C and D columns.")
            ...
            print(f"[{date_str}] DEBUG: Wrote WS_Calc E and F columns.")
            ...
            print(f"[{date_str}] DEBUG: Finished writing PR_Calc data.")
            ...
            print(f"[{date_str}] DEBUG: Finished writing Inverter_data powers.")
            ...
            # Correct columns AA, AB, AC headers on Row 14 to represent Average, Maximum, and Minimum of TX1
            ws_calc.Cells(14, 27).Value = "TX1 - Average"
            ws_calc.Cells(14, 28).Value = "TX1 - Maximum"
            ws_calc.Cells(14, 29).Value = "TX1 - Minimum"
            print(f"[{date_str}] DEBUG: Finished updating titles and side tables. Restoring calculation to automatic and saving daily workbook...")
            ...
            wb_daily.Save()
            print(f"[{date_str}] Daily file '{daily_filename}' saved successfully via Excel COM (keeping open for Mother linking)!")
        except Exception as ex:
            print(f"[{date_str}] Error updating daily file via Excel COM: {ex}")
        ...
        if not os.path.exists(mother_path):
            ...
            if orig_mothers:
                ...
                print(f"[{date_str}] Copying and initializing Mother template via Excel COM: '{os.path.basename(orig_mother_path)}' -> '{expected_mother_filename}'")
                ...
                try:
                    ...
                    for r in range(5, 5 + num_days):
                        ...
                        if not os.path.exists(chk_daily_path):
                            # Daily file does not exist yet! Clear all columns from B to AR (cols 2 to 44)
                            for col in range(2, 45):
                                ws_mother.Cells(r, col).Value = None
                        else:
                            ...
                            for col in range(2, 45):
                                ...
                                if isinstance(f_text, str) and '!' in f_text:
                                    prefix = f_text.split('!')[0]
                                    if col == 2: addr = "$I$111"
                                    elif col == 3: addr = "$M$111"
                                    elif col == 4: addr = "$BL$111*100"
                                    elif col == 6: addr = "$AD$111"
                                    elif col == 7: addr = "$AT$111"
                                    elif col == 8: addr = "$BJ$111"
                                    elif 9 <= col <= 20:
                                        import openpyxl
                                        ...
                                        import openpyxl
                                        daily_col_letter = openpyxl.utils.get_column_letter(col + 10)
                                        addr = f"{daily_col_letter}$111"
                                    elif 33 <= col <= 44:
                                        import openpyxl
                                        daily_col_letter = openpyxl.utils.get_column_letter(col + 14)
                                        addr = f"{daily_col_letter}$111"
                    ...
                    print(f"[{date_str}] Mother file '{expected_mother_filename}' initialized perfectly with {num_days} days using Excel COM.")
                except Exception as ex:
                    print(f"[{date_str}] Error initializing Mother file via Excel COM: {ex}")
                    if 'wb_mother' in locals() and wb_mother:
                        wb_mother.Close(SaveChanges=False)
                finally:
                    pass
            else:
                print(f"[{date_str}] Warning: No mother template found in '{original_format_dir}'")
        ...
        if os.path.exists(mother_path):
            ...
            try:
                ...
                if match_row:
                    print(f"[{date_str}] Writing PR SCADA static value and restoring formulas for row {match_row} in Mother file via Excel COM.")
                    ...
                    if orig_mothers:
                        orig_mother_path = orig_mothers[0]
                        orig_mother_path_abs = os.path.abspath(orig_mother_path).replace('/', '\\')
                        wb_temp = excel.Workbooks.Open(orig_mother_path_abs, UpdateLinks=0, ReadOnly=True)
                        ws_temp = wb_temp.Sheets('PR_Calc')
                        
                        temp_row = match_row if match_row <= 34 else 34
                        ...
                        for col in range(2, 45):
                            if col == 5: continue
                            temp_formula = ws_temp.Cells(temp_row, col).Formula
                            if isinstance(temp_formula, str) and '!' in temp_formula:
                                import openpyxl
                                if col == 2: addr = "$I$111"
                                elif col == 3: addr = "$M$111"
                                elif col == 4: addr = "$BL$111*100"
                                ...
                                ws_mother.Cells(match_row, col).Value = ws_temp.Cells(temp_row, col).Value
                                
                        wb_temp.Close(SaveChanges=False)
                else:
                    print(f"[{date_str}] Warning: Could not find matching date {dt_comp.date()} in Mother file to update formulas.")
                ...
                print(f"[{date_str}] Error updating Mother file row via Excel COM: {ex}")
                if 'wb_mother' in locals() and wb_mother:
                    wb_mother.Close(SaveChanges=False)
            finally:
                if 'wb_daily' in locals() and wb_daily:
        ...
        try:
            ...
            if is_batch:
                ...
                for idx, day_str in enumerate(numerical_subdirs):
                    ...
                    status_text = f"Batch mode: Processing day {day_val} of {len(numerical_subdirs)}..."
                    ...
                    try:
                        ...
                        raise ValueError(f"Error on Day {day_str}: {day_err}")
                        
                if processed_count == 0:
                    status_msg = f"All {skipped_count} days already processed. Check box to reprocess."
                ...
                success_msg = f"Batch complete! Processed: {processed_count} days, Skipped: {skipped_count} days."
                ...
                self.root.after(0, lambda: self.lbl_status.config(text="Calculating single day...", foreground=self.warn_color))
                ...
                success_msg = f"Calculation finished! Generated daily file & updated Mother file."
                self.root.after(0, lambda m=success_msg: self.lbl_status.config(text=m, foreground=self.success_color))
                self.root.after(0, self.update_ui_on_success)
    ...
    def update_ui_on_success(self):
        ...
        self.lbl_irrad_summary.config(
            text=f"Total daily irradiation: {res['h_sum_kwh']:.4f} kWh/m² (Irradiance Average > {self.threshold_var.get()} W/m²)"
        )
        ...
        self.lbl_status.config(text="Calculation completed successfully!", foreground=self.success_color)
        messagebox.showinfo("Success", f"PR calculation for {res['date_str']} finished successfully!\n\nAverage Inverter PR: {res['avg_inv_pr']:.3f}%")
        
    def update_ui_on_failure(self, error_message):
        self.btn_calculate.config(state="normal")
        self.btn_export.config(state="disabled")
        self.lbl_status.config(text="Calculation failed!", foreground="red")
        messagebox.showerror("Calculation Error", error_message)
        
    def export_to_excel(self):
        if self.df_result is None or self.calc_results is None:
            messagebox.showerror("Error", "No calculation results available to export!")
            return
        ...
        try:
            self.lbl_export_status.config(text="Exporting... please wait...", foreground=self.warn_color)
            self.btn_export.config(state="disabled")
            ...
            self.lbl_export_status.config(text="Export failed!", foreground="red")
        ...
        try:
            ...
            with pd.ExcelWriter(save_path, engine='openpyxl') as writer:
                ...
                summary_data = {
                    "Metric": [
                        "Calculation Date",
                        "Total Daily Irradiation (kWh/m²)",
                        "Monthly PVSyst PR Target",
                        "Min Irradiance Threshold (W/m²)",
                        "RAW Uncompensated PR (%)",
                        "Compensated RAW PR (%)",
                        "Average of 36 Comp Inverter PRs (%)"
                    ],
                    "Value": [
                        res["date_str"],
                        res["h_sum_kwh"],
                ...
                inverter_cols = ["Inverter ID", "Transformer", "Nominal DC (kW)", "Actual Energy (kWh)", "Energy Loss (kWh)", "Compensated PR (%)"]
                df_inv_prs = pd.DataFrame(res["inverter_table_data"], columns=inverter_cols)
                df_inv_prs.to_excel(writer, sheet_name="Inverter_PRs", index=False)
                
                # 3. Complete Timeslot Data
                # Pick core columns to avoid extremely fragmented or confusing columns
                cols_to_export = [
    ...
    def on_export_success(self, save_path):
        self.btn_export.config(state="normal")
        self.lbl_export_status.config(text="Export completed successfully!", foreground=self.success_color)
        messagebox.showinfo("Export Success", f"Detailed PR report saved to:\n\n{save_path}")
