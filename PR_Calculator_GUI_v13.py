...
class PRCalculatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Calcolatore Performance Ratio (PR) Fotovoltaico Mazara 01 - v13.0")
        ...
        print(">>> GET SRL - Motore di calcolo Performance Ratio inizializzato (v13).")
    ...
    def create_layout(self):
        ...
        # Version badge
        version_badge = tk.Label(header_inner, text="v13.0", bg="#e8f0fe", fg="#1a73e8", font=("Segoe UI Semibold", 9), padx=8, pady=2)
        ...
        self.spin_day_from = ttk.Spinbox(scope_row, from_=1, to=31, width=4,
                                         textvariable=self.day_from_var, font=("Segoe UI", 10))
        self.lbl_day_dash = tk.Label(scope_row, text="-", bg="#ffffff", fg=self.muted_text,
                                     font=("Segoe UI", 10))
        self.spin_day_to = ttk.Spinbox(scope_row, from_=1, to=31, width=4,
                                       textvariable=self.day_to_var, font=("Segoe UI", 10))
        self.spin_day_from.pack(side="left")
        self.lbl_day_dash.pack(side="left", padx=3)
        self.spin_day_to.pack(side="left")
    ...
    def _on_day_scope_change(self):
        """Enable the Da/A spinboxes only for the 'Intervallo' scope; update the hint."""
        scope = self.day_scope_var.get()
        state = "normal" if scope == "intervallo" else "disabled"
        try:
            self.spin_day_from.config(state=state)
            self.spin_day_to.config(state=state)
    ...
    def _find_vcom_folder_for_day(self, day_path):
        """Check if a day_path has a 'vcom' or 'VCOM' directory that contains VCOM AC and Prod CSV files."""
        for name in ["vcom", "VCOM"]:
            v_dir = os.path.join(day_path, name)
            if os.path.exists(v_dir) and os.path.isdir(v_dir):
                try:
                    files = os.listdir(v_dir)
                    has_ac = any("Potenza_AC" in f and f.endswith(".csv") for f in files)
                    has_prod = any("Produzione_energetica" in f and f.endswith(".csv") for f in files)
                    if has_ac and has_prod:
                        return v_dir
                except Exception:
                    pass
        return None
            ...
            if is_batch:
                ...
                for day_str in numerical_subdirs:
                    ...
                    if miss:
                        ...
                        # Check if VCOM folder is available
